...
const labelInfoCSV = path.resolve(__dirname, `../files - new/labels info.csv`);
const itemLabelCSV = path.resolve(__dirname, `../files - new/item labels.csv`);
...
const fabricAndCareAttribute = '8000df9a-1ac1-4d9b-bb00-a2d68f50b603'
///////////////////////////////////////////////////////////////////////////////////
...
async function processAttributeNamesCSV() {
    return new Promise((resolve, reject) => {
        const stream = fs.createReadStream(itemLabelCSV)
            .pipe(fastcsv.parse({ headers: true, trim: true }))
            .on('error', error => {
                reject(error);
            })
            .on('data', row => {
                itemLabelLookup[row['Product/RM Code']] = row['Label Code']
            })
            .on('end', () => {
                resolve();
            });
...
async function processAttributeNamesCSV() {
    return new Promise((resolve, reject) => {
        const stream = fs.createReadStream(labelInfoCSV)
            .pipe(fastcsv.parse({ headers: true, trim: true }))
            .on('error', error => {
                reject(error);
            })
            .on('data', row => {
                labelInfoLookup[row['Label Code']] = row['Label Name']
            })
            .on('end', () => {
                resolve();
            });
    ...
    async function processItem(parent){
        try{
            ...
            for (const child of Object.keys(parentProduct.variants)) {
                ...
                if(labelInfoLookup?.[itemLabelLookup?.[parent.split('-')[0]]] != undefined){
                    attributes.push({
                        "itemAttributeId": fabricAndCareAttribute,
                        "value": labelInfoLookup?.[itemLabelLookup?.[parent.split('-')[0]]]
                    });
                }
                ...
                let childData = {
                    ...
                    appendAttributes: true,
                    harmonyCode: variant?.attributes?.FM?.['Intrastat'] || variant?.attributes?.Protex?.['Commodity Code'],
...
async function run() {
    ...
    // try {
        ...
        fs.writeFileSync('./productDump.JSON', JSON.stringify(productData, null, 2));
