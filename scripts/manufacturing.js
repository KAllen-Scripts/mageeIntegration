...
let manufacturingSheet = '../files/manufacturing/SO_20241216155051(in).csv';
...
async function processOrders(){
    ...
    for (const order of orderArr){
        try{
        // if (existingRuns[order.number] != undefined){
        //     console.log('Exists')
        //     continue
        // }
        ...
        let orderJSON = {
            ...
            "deliveryLocationId": 'ff2930d7-5889-4bfc-8dd6-54bd64ef0a45',
            items: []
        }
        ...
        for (const item of order.items){
            ...
            orderJSON.items.push({
                ...
                "quantityExpected": parseInt(item['order quantity'])
            })
        ...
        if(!problemOrder){
            try{
                await common.requester('post', `https://api.stok.ly/v0/manufacturing-runs`, {...orderJSON, additionalCosts}, 0)
            } catch (err) {
                console.log(err)
            }
        }else{
